...
proc bind*(server: TCPServer, host: string, port: Port) =
    result.host = newSockaddr_in(host, port)
    result.port = port
    checkError uv_tcp_bind(addr server.server, cast[ptr SockAddr](addr result.host), 0)
