...
# Had to make a fork of this module because the original one didn't allow to directly access the queue without using futures.
...
proc writeFirst*[T](future: FutureStream[T], value: T) =
  ## Writes the specified value inside the specified future stream as the first value.
  ##
  ## Use only if you know what you are doing.

  # TODO: Implement limiting of the streams storage to prevent it growing
  # infinitely when no reads are occurring.
  future.queue.addFirst(value)
  #if not future.cb.isNil: future.cb()
  #result.complete()
...
proc tryRead*[T](future: FutureStream[T]): (bool, T) =
  ## Returns the oldest value stored inside the `FutureStream` or `false`
  ## if the stream is empty.
  ##
  ## This function will remove the data that was returned from the underlying
  ## `FutureStream`.
  if future.queue.len > 0:
    result = (true, future.queue.popFirst())
  else:
    result[0] = false
